...
d=1;                     % Signal Degradation rate of 
...
Ranking = NaN(net_size,num_cmn_points); %will hold the values for ordered ranking of Z_values based on pop size, weights, etc from 1
Ranking(1,:) = 1;
        ...
        % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
         ...
         % - - - - - - - - - - - - - L I N E A R   R E S P O N S E   M A T R I X - - - - - - - - - - - - - %   
            for node_index = 1:net_size   
                ...
                %storing cell population and weight sum values 
                 Population_values(realizations,node_index,common_node_count) = NN(1).cell_pop(node_index);
                 Weights_sum_values(realizations,node_index,common_node_count) = sum(abs(NN(1).weights(:,node_index)));
            end
            
    end
end

%Now create a copy of reordered HD_values by population, weights
...
%calculating the NHDs of the total network
HD_values_tot_ord_pop = (HD_values_1_ord_pop + HD_values_2_ord_pop)/2;
HD_values_tot_ord_weight = (HD_values_1_ord_weight + HD_values_2_ord_weight)/2;
...
function HD1_vs_Pop(Ranking_pure,Z_0CMN,Z_1CMN,Z_2CMN,err_0CMN,err_1CMN,err_2CMN,pert_size)
    ...
    title(['Perturbation Size: ' num2str(pert_size*100) '%'])
end
...
function HDtot_vs_Pop(Ranking_pure,Z_0CMN,Z_1CMN,Z_2CMN,err_0CMN,err_1CMN,err_2CMN,pert_size)
    ...
    scatter(Ranking_pure(:,2),Z_1CMN);
    hold on;
    scatter(Ranking_pure(:,3),Z_2CMN,"MarkerFaceColor",[0.9290 0.6940 0.1250]);
    hold on;
    ...
    errorbar(Ranking_pure(:,2),Z_1CMN,err_1CMN,'r');
    hold on;
    errorbar(Ranking_pure(:,3),Z_2CMN,err_2CMN,"MarkerFaceColor",[0.9290 0.6940 0.1250]);
