% This code is removing a node and plotting the graphs for -100% cell number perturbation 
...
%n_realiz=1;
n_realiz=en_pert ;% The total number of realizations is just
...
for realizations=1:n_realiz
    ...
    pert_obj_1=structural_perturbation(final_NN(1),final_NN(2),node_matrix,'Common Node Random Cell Number Perturbation',2,pert_radius,time,delta_t,f);
    ...
    i=find(pert_stat_pert.pert_size==-1); 
bar_hd(n_common+1,1)=pert_stat_pert.pert_data(i).en_mean_hd;
bar_hd(n_common+1,2)=pert_stat_unpert.pert_data(i).en_mean_hd;
stdd(n_common+1,1)=pert_stat_pert.pert_data(i).std;
stdd(n_common+1,2)=pert_stat_unpert.pert_data(i).std;



end
f3=figure(3);
%bar([0 1 2],bar_hd(:,1),"stacked");hold on;
bar([0 1 2],bar_hd(:,1));hold on;
%er = errorbar([0 1 2],bar_hd(:,1),stdd(:,1),"LineWidth",1,"LineStyle","none");hold on;
bar([0 1 2],bar_hd(:,2));hold on;
%er_1 = errorbar([0 1 2],bar_hd(:,2),stdd(:,2),"LineWidth",1,"LineStyle","none");

xlabel("Number of Common Nodes");
ylabel("< NHD >");
title("90% Signal Perturbation")
legend("Perturbed Layer","Unperturbed Layer");
