...
% - - - - - - - - - - - - - S T A T I S T I C S - - - - - - - - - - - - - -
pert_stat_cn=perturbation_statistics(net_size,pert_radius);
pert_stat_target_max_cn=perturbation_statistics(net_size,pert_radius);
...
for realizations=1:n_realiz
    ...
    cn_pert_size_2=pert_obj_2.pert_size;
 
    % Perturbing a random node in the network with a random perturbation
    [nhd_p,nhd_up,pert_obj_1]=perturb_purenode(pert_obj_1);
    %if pert_obj_1.pert_size==0 && (nhd_p >0 || nhd_up>0)
    %    for i=1:10
    %        plot(pert_obj_1.final_pert_nn.sig_time(i,:));hold on;
    %    end
    %    hold off;
    %end
    ...
    [n_HD_pert_lay,n_HD_unpert_lay,pert_obj_2]=perturb_targetnode(pert_obj_2,perturb_node);

    %norm_sig_HD_pert_lay=sig_HD_pert_lay/net_size;
    pert_stat_cn=collecting_data(pert_stat_cn,nhd_p,cn_pert_size_1);
    pert_stat_target_max_cn=collecting_data(pert_stat_target_max_cn,n_HD_pert_lay,cn_pert_size_2);


    

    %Here we assign the normalized hd in the perturbation statistics object
end
f1=figure(1);
hold on;
pert_stat_cn.plotter("Cell Number Perturbations",'b'); 
pert_stat_target_max_cn.plotter("Cell Number Perturbations",'r');
